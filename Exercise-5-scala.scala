...
val student_name: String = "anh_chu"
...
val df_parquet: DataFrame = spark.read.parquet(source_parquet_folder)
...
val sorted_df_csv: DataFrame = df_csv.filter("City == 'Los Angeles'").orderBy(desc("Start_Time")).limit(73)

// Create a data frame that holds some new rows that will be appended to the storage
val df_new_rows: DataFrame = sorted_df_csv.withColumn("ID", concat(col("ID"), lit("_Z1")))
...
val df_new_parquet: DataFrame = spark.read.parquet(target_file_parquet)
...
val updatedIdColumn = regexp_replace(col("ID"), "_Z1$", "_Z2")
val addedColumn1 = concat(lit("prefix-"), col("City"))
val addedColumn2 = lit("New column")
val temperatureCColumn = ((col("Temperature_F") - lit(32)) * lit(5) / lit(9)).alias("Temperature_C")

// Some new rows that have different columns
val df_modified: DataFrame = df_new_rows
  .withColumn("ID", updatedIdColumn)
  .withColumn("Temperature_C", temperatureCColumn)
  .withColumn("AddedColumn1", addedColumn1)
  .withColumn("AddedColumn2", addedColumn2)
...
val modified_parquet_df: DataFrame = spark.read
  .parquet(target_file_parquet)
...
// MAGIC %md
// MAGIC **Did you get similar output for the data in CSV storage? If not, what was the difference?** Yes, I got similar output
// MAGIC
// MAGIC **What is your explanation/guess for why the CSV seems broken and the schema cannot be inferred anymore?** My guess is there are conflicts between the two appended schema and the dataframe instead of creating 11 columns, it uses 8 columns of the first appended dataframe. The second dataframe appended while using 8 columns of the first dataframe which result in overwrite the data to the different schema instead of creating new ones based on its schema. 
// MAGIC
// MAGIC **Did you get similar output for the data in Parquet storage, and which of the 2 alternatives? If not, what was the difference?** Yes, I got the first alternative
// MAGIC
// MAGIC **What is your explanation/guess for why not all 11 distinct columns are included in the data frame in the Parquet case?** There are conflicts in the schema when append to the target folder with 2 different dataframe which have 2 different schemas. As I observed, the result used 8 columns of the first appended dataframe and the second one append to the existing dataframe using same 8 columns.
...
deltatable.as("orig")
    .merge(df_delta_update.as("updates"), "orig.ID == updates.ID")
...
deltatable.update(Map("Temperature_C" -> round(((col("Temperature_F") - 32) * 5 / 9), 1)))

// code for removing rows where the temperature is below -12 Celsius degrees from the deltatable
deltatable.delete(
  condition = expr("Temperature_C < -12")
)
