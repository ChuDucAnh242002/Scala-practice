...
val weatherSample1: Array[Row] = weatherDF.head(7)
...
val weatherSample2: Array[Row] = weatherDF.tail(6)
...
val minTempdf: DataFrame = weatherDF.agg(min("temperature_min").alias("min_temperature"))
val maxTempdf: DataFrame = weatherDF.agg(max("temperature_max").alias("max_temperature"))

val minTemp: Double = minTempdf.first().getDouble(0)
val maxTemp: Double = maxTempdf.first().getDouble(0)
...
val temperatureDF1: DataFrame = weatherDFWithYear.groupBy("year").agg(
  min("temperature_min").alias("temperature_min"),
  max("temperature_max").alias("temperature_max")
)

val temperatureDF: DataFrame = temperatureDF1.sort(desc("year"))
...
val df: DataFrame = sortedWeatherDF.groupBy("year", "country").agg(
  count("*").alias("entries"),
  round(min("temperature_min"), 1).alias("temperature_min"),
  round(max("temperature_max"), 1).alias("temperature_max"),
  round(avg("snow_depth")).alias("snow_depth_avg")
)

val task6DF: DataFrame = df.sort(desc("year"), asc("country"))
...
val dfF6 = task6DF.filter($"year" === 2016 && $"country" === "Finland" )
val dfS7 = task6DF.filter($"year" === 2017 && $"country" === "Sweden")
val dfN8 = task6DF.filter($"year" === 2018 && $"country" === "Norway")
val df5 = task6DF.filter($"year" === 2015).agg(
  round(avg("snow_depth_avg")).alias("snow_depth")
)

val min2016: Double = dfF6.first().getDouble(3)
val max2017: Double = dfS7.first().getDouble(4) 
val difference2018: Double = dfN8.first().getDouble(4) - dfN8.first().getDouble(3) 
val snow2015: Double = df5.first().getDouble(0)
...
val daysBelowMinus10DF: DataFrame = weatherDFWithYear.filter($"temperature_avg" < -10).groupBy("year").agg(count("date").alias("count")).sort(asc("year"))

println("The number of days the average temperature in Finland was below -10 °C:")
daysBelowMinus10DF.show()

val daysAbove5DF: DataFrame = weatherDFWithYear.filter($"temperature_avg" > 5 && $"snow_depth" > 100).groupBy("country").agg(count("date").alias("count")).sort(asc("country"))

println("The number of days the temperature in each country was above +5 °C when snow depth was above 100 mm:")
daysAbove5DF.show()

val differenceDF: DataFrame = weatherDF.withColumn("temperature_diff", round($"temperature_max" - $"temperature_min", 2))
val differenceDaysDF: DataFrame = differenceDF.filter($"country" === "Finland").select("date", "temperature_diff").sort(desc("temperature_diff"))

println("The top 10 days in Finland with the largest temperature difference:")
differenceDaysDF.show(10)
