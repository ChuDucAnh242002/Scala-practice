...
val file_parquet = "abfss://shared@tunics320f2024gen2.dfs.core.windows.net/exercises/ex3/procem_iotdb.parquet/data.parquet"
...
val roundTimeDF: DataFrame = procemDF.withColumn("Time", from_utc_timestamp( from_unixtime($"Time" / 1000,"yyyy-MM-dd HH:00:00"), "Europe/Helsinki"))
val producedEnergyDF: DataFrame = roundTimeDF.groupBy("Time").agg(
  avg("Temperature").alias("AvgTemperature"),
  (avg("SolarPower") / 1000).alias("ProducedEnergy"),
  ((avg("WaterCooling01Power") + avg("WaterCooling02Power") + avg("VentilationPower")) / 1000).alias("ConsumedEnergy"),
  first("ElectricityPrice").alias("Price")
)

val hourlyDF: DataFrame = producedEnergyDF.orderBy(asc("Time"))
...
val dailyFormatDF: DataFrame = hourlyDF.withColumn("Time", date_format($"Time", "yyyy-MM-dd"))
val dailyDF: DataFrame = dailyFormatDF.groupBy("Time").agg(
  round(avg("AvgTemperature"),2).alias("AvgTemperature"),
  round(sum("ProducedEnergy"),2).alias("ProducedEnergy"),
  round(sum("ConsumedEnergy"),2).alias("ConsumedEnergy2"),
  round((sum($"Price" * ($"ConsumedEnergy" - $"ProducedEnergy")) / 1000),2).alias("DailyCost")
).orderBy("Time")

dailyDF.show()
val totalPrice: Double = dailyDF.select("DailyCost").agg(sum("DailyCost")).as[Double].first()
...
val folderPath = "abfss://shared@tunics320f2024gen2.dfs.core.windows.net/exercises/ex3/wiki"
val textFiles = Seq(
      "Artifical_intelligence.txt",
...
val textPaths = textFiles.map(fileName => folderPath + fileName)

val wikitextsRDD: RDD[String] = spark.sparkContext.textFile(folderPath).filter(line => line.trim.nonEmpty)
...
val wordsRDD: RDD[String] = wikitextsRDD
  ...
  .filter(word => word.nonEmpty && !word.exists(_.isDigit))
...
val wordCountRDD: RDD[(String, Int)] = wordsRDD
  .map(word => (word, 1)) // Map each word to a count of 1
...
val commonsRDD = wordCountRDD
  .filter{case (word, count) => word.length == 7 && word.startsWith("s")}
  .sortBy(_._2, ascending=false)

val (askedWord: String, wordCount: Int) = commonsRDD.first()
...
val wikitextsDataset: Dataset[String] = spark.read.textFile(folderPath).filter(line => line.trim.nonEmpty)
...
val wordCountDataset: Dataset[WordCount] = wordsDataset
  ...
  .mapGroups { case (word, occurrences) =>
    WordCount(word, occurrences.size) // Calculate count for each word
  }

println(s"First row in wordCountDataset: word: '${wordCountDataset.first().word}', count: ${wordCountDataset.first().count}")

val commonsDataset: Dataset[WordCount] = wordCountDataset
  .filter(wordCount => wordCount.word.length == 7 && wordCount.word.startsWith("s")) // Filter by length and prefix
  .orderBy(desc("count")) // Sort by count in descending order

val askedWordCount: WordCount = commonsDataset.first()
