...
val schema =
  StructType(
    ...
    StructField("label", DoubleType, false) :: Nil)

val dataDF: DataFrame = spark.createDataFrame(dataRDD, schema)

val randomsplitDF = dataDF.randomSplit(Array(0.8, 0.2), 1)
// Split the data into training and testing datasets (roughly 80% for training, 20% for testing)
val trainingDF: DataFrame = randomsplitDF(0)
val testDF: DataFrame = randomsplitDF(1)
trainingDF.sort($"X")
testDF.sort($"X")
...
val newSeq : Seq[Row] = Seq(Row(-2.8), Row(3.14), Row(9.9), Row(123.45))
val newSchema = 
  StructType(
    StructField("X", DoubleType, false) :: Nil )
val newRDD: RDD[Row] = spark.sparkContext.parallelize(newSeq)
val newDF : DataFrame = spark.createDataFrame(newRDD, newSchema)
val assembledNewDF : DataFrame = vectorAssembler.transform(newDF)
val newPredictions : DataFrame = lrModel.transform(assembledNewDF)
...
val file_csv = "abfss://shared@tunics320f2024gen2.dfs.core.windows.net/exercises/ex4/static/superstore_sales.csv"
...
val bestDaysDF: DataFrame = salesDF.groupBy("orderDate").agg(
  sum($"productPrice" * $"productCount").alias("totalSales")
...
val myStreamingIdentifier: String = "anh_chu"
...
val salesSchema : StructType = salesDF.schema
val salesStreamingDF: DataFrame = spark.readStream
  .schema(salesSchema) // Explicit schema
  .option("header", "false") // No headers in the streaming data
  .csv(myStreamingFolder) // Read CSV files from the specified folder
...
val bestDaysStreamingDF: DataFrame = salesStreamingDF.groupBy("orderDate").agg(
  sum($"productPrice" * $"productCount").alias("totalSales"),
...
val limitedStreamingDF = bestDaysStreamingDF.limit(8)

val myStreamingOutput = limitedStreamingDF.writeStream
  .format("console")
  .outputMode("complete")
  .option("checkpointLocation", s"$myStreamingFolder/checkpoint")
  .option("path", myStreamingFolder)

val myStreamingTrigger = myStreamingOutput
  .trigger(Trigger.AvailableNow())

// start the streaming query with the console format and limit the output to the first 8 rows
val myStreamingQuery: StreamingQuery = myStreamingTrigger
  .start()
